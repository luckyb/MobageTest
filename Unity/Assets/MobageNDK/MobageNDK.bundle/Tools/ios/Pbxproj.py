...
# encoding: utf-8
"""
THE ORIGINAL PBXPROJ.PY SOURCE CODE DISTRIBUTION HAS BEEN MODIFIED - ngmoco:) LLC, DeNA Co., Ltd. 2012
"""
...
class Pbxproj(object):
	...
	def get_project_target(self):
		...
		targets = []
		match = re.search('\/\* Begin PBXNativeTarget section \*\/\n((?:.|\n)+?)\/\* End PBXNativeTarget section \*\/', project_data)
		if match:
			(subtext, ) = match.groups()
			logging.info(subtext)
			match = re.search(' \*\/ = {\n[ \t]+isa = PBXNativeTarget;(?:.|\n)+?name = (["a-zA-Z0-9\.\-]+);',subtext)
			logging.info("PBXNativeTarget")
			if match:
	...
	def add_buildfile(self, name, file_ref_hash, default_guid, weak=False):
		project_data = self.get_project_data()
		...
		if buildfile_hash is None:
			...
			weakFragment = ""
			if weak: weakFragment = " settings = {ATTRIBUTES = (Weak, ); };"
		
			libfiletext = "\t\t"+buildfile_hash+" /* "+name+" in Frameworks */ = {isa = PBXBuildFile; fileRef = "+file_ref_hash+" /* "+name+" */;" + weakFragment + " };\n"
	...
	def add_framework(self, framework, weak=False):
		...
		libfile_hash = self.add_buildfile(framework, fileref_hash, tthash_base+'1', weak)
	...
	def add_lib(self, framework):
		...
		libfile_hash = self.add_buildfile(framework, fileref_hash, hash_base+'1')
		if not self.add_file_to_frameworks(framework, fileref_hash):
			return False
